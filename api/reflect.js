export default async function handler(req, res) {
  ...
  try {
      ...
      if (!response.ok) {
              ...
              return res.status(502).json({ error: 'AI unavailable', details: `API error ${response.status}: ${errBody}` });
      }
      ...
      if (!clean) {
              ...
              return res.status(502).json({ error: 'AI unavailable', details: 'Empty response from AI' });
      }
